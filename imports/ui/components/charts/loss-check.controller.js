...
controller('LossCheckController', [
  '$scope', '$mdToast', '$cookies', '$translate', '$meteor', '$stateParams', '$location', 'LossCheckService', 'ExportDataService', ($scope, $mdToast, $cookies, $translate, $meteor, $stateParams, $location, LossCheckService, ExportDataService) => {
    $scope.partNumbers = [''].concat(_.keys(Settings.partNumbers));
    $scope.intervals = _.keys(Settings.lossintervals);
    $scope.partNumber = $stateParams.pnum;
    ...
    function processRows() {
      ...
      _.each(racks, (rack) => {
        ...
        $meteor.call('losses', $scope.lossChartType, $scope.partNumber,
          $scope.manufacturer, $scope.intervalValue, $scope.range, rack, $scope.reworkOnly, $scope.device, $scope.paramsFail).then(
          function(losses) {
            listOfSerials = [];
            _.each(losses, (loss) => {
    ...
    function exportData(tt, doc) {
      ...
      $meteor.call('exportData', tt ? doc.ids : listOfSerials, [tt], null, null, null, 'F').then(
        function(data) {
          if (data.length === 0) {
